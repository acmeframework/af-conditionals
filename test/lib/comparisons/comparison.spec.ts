  ...
  describe('Tests the functionality of the class', function () {
    ...
    equalComparisons.forEach((testItems: TestItem) => {
      it('using compare, tests ' + testItems[0] + ' for equality', function () {
        expect(c.compare(testItems[1], testItems[2])).to.eq(COMPARE_EQUAL);
      });
    });
    ...
    greaterThanComparisons.forEach((testItems: TestItem) => {
      it(
        'using compare, tests ' + testItems[0] + ' for greater than',
        function () {
          expect(c.compare(testItems[1], testItems[2])).to.eq(
            COMPARE_GREATER_THAN
          );
        }
      );
    ...
    lessThanComparisons.forEach((testItems: TestItem) => {
      it(
        'using compare, tests ' + testItems[0] + ' for less than',
        function () {
          expect(c.compare(testItems[1], testItems[2])).to.eq(
            COMPARE_LESS_THAN
          );
    ...
    it('tests outside cases', function () {
      expect(c.compare(TEST_FUNCTION_1, TEST_FUNCTION_1)).to.eq(COMPARE_EQUAL);

      expect(c.compare(TEST_FUNCTION_1, TEST_FUNCTION_2)).to.not.eq(
        COMPARE_EQUAL
      );
    });

    it('tests that strings are properly handled when strict is false', function () {
      expect(c.compare(TEST_STRING_1, TEST_STRING_2, false)).to.eq(
        COMPARE_LESS_THAN
      );
    });
    ...
    it('uses assert/test to test comparisons', function () {
      ...
      expect(function () {
        assert_comparison(TEST_STRING_1, TEST_STRING_2, c, [COMPARE_EQUAL]);
      }).to.throw(TypeError);

      expect(c.test(TEST_STRING_1, TEST_STRING_1)).to.be.true;

      expect(c.test(TEST_STRING_1, TEST_STRING_2, COMPARE_EQUAL)).to.be.false;
    });

    it('uses compare (function) to test comparisons', function () {
      expect(compare(TEST_STRING_1, TEST_STRING_1)).to.eq(COMPARE_EQUAL);
      expect(compare(TEST_STRING_1, TEST_STRING_2, false)).to.eq(
        COMPARE_LESS_THAN
      );
    });
  });
